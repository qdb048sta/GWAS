...
*version 1.2
cap program drop  semipar4
program define semipar4, eclass
version 10.1

if replay()& "`e(cmd)'"=="semipar4" {
        ereturn  display
exit
}
...
local kernel="gaussian"
                ...
                cap drop dup
                qui sort `nonpar'
                quietly by `nonpar':  gen dup = cond(_N==1,0,_n)
                cap drop uniqx
                qui gen uniqx = `nonpar' if dup<=1
...
capture qui kdensity `nonpar' `wgt', nograph gen(`aa' `nonpardd') at(uniqx)
qui sum `nonpardd'
...
tempvar hat`var'
capture qui lpoly `var' `nonpar' if `touse', degree(`degree') gen(`hat`var'') at(uniqx) nograph kernel(`kernel')
cap drop `hat`var''1
qui bysort `nonpar': carryforward `hat`var'', gen(`hat`var''1)
qui replace `hat`var''=`hat`var''1
...
if "`generate'"!="" {
        ...
        if `nw'==1 {  
                ...
                qui sum  `nonpar'
                cap lpoly `res2' `nonpar' `wgt' if `touse' , degree(`degree') gen(`marker') at(uniqx) legend(off) kernel(`kernel')  nograph
                
                cap drop `marker'1
                qui bysort `nonpar': carryforward `marker', gen(`marker'1)
                qui replace `marker'=`marker'1
...
cap lpoly `res2' `nonpar' `wgt' if `touse', degree(`degree') nograph at(uniqx) kernel(`kernel') 
}
...
lpoly `res2' `nonpar' `wgt' if `touse', gen(`yhat0') at(uniqx) nograph kernel(`kernel') degree(`degree') 
