...
global grid = "$G $levels"
        ...
        use "`s'_`diff'_SEX`s'_Work`w'_BB`bb'_PV`pv'_file1.dta", replace
        qui drop if h_grid==.
        cap drop _merge
        merge 1:1 h_grid using "`s'_`diff'_SEX`s'_Work`w'_BB`bb'_PV`pv'_file0.dta", nogen
        save `s'_`diff'_BB`bb'_PV`pv'_file3.dta, replace
        ...
        tw  (rcap  mar_lbh_b`ci'  mar_lbh_t`ci'  h_grid )(line mar_lbh   h_grid) (line beta_iv h_grid), title(Marginal_SEX`s'_Work`w'_BB`bb'_PV`pv')
        graph export `s'_marginal_`diff'_SEX`s'_Work`w'_BB`bb'_PV`pv'_rcap.pdf, as(pdf) replace
        tw  (rarea  mar_lbh_b`ci'  mar_lbh_t`ci'  h_grid ) (line mar_lbh   h_grid)(line beta_iv h_grid), title(Marginal_SEX`s'_Work`w'_BB`bb'_PV`pv')
